...
with open("check_audio_files.txt", "w") as f:
    for order in audio_files:
        f.write(
            (order.split(".mp3")[0]).split(
                "/home/phoenix/llama/llama.lisp/src/AI/stt/audio_dataset_torch_audio/"
            )[1]
            + "\n"
        )
...
with open("audio_prompt_files.txt", "w") as f:
    for order in audio_prompt_files:
        f.write(
            (order.split(".txt")[0]).split(
                "/home/phoenix/llama/llama.lisp/src/AI/stt/text_dataset_torch_text/"
            )[1]
            + "\n"
        )
