(c-lisp
    ...
    (define ((initialise void)
             (x (ptr float)) (w (ptr float)) (column int) (row int))
        ...
        (for ((set i 0) (lt i row) (set i (add i 1)))
            ...
            (for ((set j 0) (lt j column) (set j (add j 1)))
                (store (ptradd w (add j (mul i row)))
                       (fadd column_float row_float ))
    ...
    (define ((mat_mul void)
             (xout (ptr float)) (x (ptr float)) (w (ptr float))
             (n int) (d int))
        (declare i int)
        (for ((set i 0) (lt i d) (set i (add i 1)))
            ...
            (for ((set j 0) (lt j n) (set j (add j 1)))
                (set val (fadd val (fmul
                    (load (ptradd w (add (mul i n) j)))
                    (load (ptradd x j))))))
