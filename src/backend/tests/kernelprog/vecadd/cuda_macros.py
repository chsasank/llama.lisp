...
cu_ipc_mem_handle = POINTER(c_byte)
cu_uuid = POINTER(c_byte)
...
def get_cuda_signatures():
    ...
    signatures = []
    for fn_name, proto in API_PROTOTYPES.items():
        fn_out = proto[0]
        args = [[f"arg-{idx}", typ] for idx, typ in enumerate(proto[1:])]
        signature = ["define", [[fn_name, fn_out]] + args]
...
printf_signature = [
    "define",
    [["printf", "int"], ["fmt", c_char_p], ["data", "int"]],
]
...
def void_ptr_to(obj):
    return ["bitcast", ["ptr-to", obj], c_void_p]
